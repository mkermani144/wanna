...
it('should be a <div />', () => {
  ...
  expect(settings.is('div.Settings')).toEqual(true);
});
it('should have 4 <ListItem />', () => {
  const settings = getActual();
  expect(settings.find('ListItem').length).toEqual(4);
});
it('should have 4 <Divider />', () => {
  const settings = getActual();
  expect(settings.find('Divider').length).toEqual(4);
});
it('should have a <CalendarSystemDialog />', () => {
  const settings = getActual();
  expect(settings.find('CalendarSystemDialog').length).toEqual(1);
});
it('should have a <FirstDayOfWeekDialog />', () => {
  const settings = getActual();
  expect(settings.find('FirstDayOfWeekDialog').length).toEqual(1);
});
...
it('should set left margin based on props', () => {
  ...
  expect(settings.prop('style').marginLeft).toEqual(56);
});
it('should set calendar system list item secondaryText based on props', () => {
  const settings = getActual({
    calendarSystem: 'fa-IR',
  });
  expect(settings.find('ListItem').at(0).prop('secondaryText')).toEqual('fa-IR');
});
it('should set first day of week list item secondaryText based on props', () => {
  const settings = getActual({
    firstDayOfWeek: 6,
  });
  expect(settings.find('ListItem').at(1).prop('secondaryText')).toEqual('Saturday');
});
it('should set fullscreen checkbox defaultChecked based on props', () => {
  const settings = getActual({
    fullscreen: true,
  });
  expect(settings.find('ListItem').at(2).prop('leftCheckbox').props.defaultChecked).toEqual(true);
});
it('should set show not yet tasks checkbox defaultChecked based on props', () => {
  const settings = getActual({
    showNotYetTasks: false,
  });
  expect(settings.find('ListItem').at(3).prop('leftCheckbox').props.defaultChecked).toEqual(false);
});
it('should set calendar system dialog calendarSystem based on props', () => {
  const settings = getActual({
    calendarSystem: 'fa-IR',
  });
  expect(settings.find('CalendarSystemDialog').prop('calendarSystem')).toEqual('fa-IR');
});
it('should set first day of week dialog firstDayOfWeek based on props', () => {
  const settings = getActual({
    firstDayOfWeek: 6,
  });
  expect(settings.find('FirstDayOfWeekDialog').prop('firstDayOfWeek')).toEqual(6);
});
